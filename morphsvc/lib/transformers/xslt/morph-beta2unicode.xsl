  ...
  <xsl:variable name="s_betaUniTable"
    ...
    <xsl:template match="entry">
      <xsl:variable name="lemma">
        <xsl:call-template name="normalize_lemma"/>
      </xsl:variable>
...
<!--
  ...
  <xsl:template name="strip-trailing">
    <xsl:param name="a_in"/>
    <xsl:param name="a_toStrip" select="'0123456789_'"/>
  ...
  <!-- normalize the lemma to preserve caps, strip vowel length and use x02bc for apos -->
  <xsl:template name="normalize_lemma">
    <xsl:variable name="stripped">
      <xsl:call-template name="beta-strip">
        <xsl:with-param name="a_in" select="dict[1]/hdwd[1]"/>
    ...
    <xsl:value-of select="translate($lemma,'&#x1FBD;','&#x02BC;')"></xsl:value-of>
  </xsl:template>
</xsl:stylesheet>
